...
if __name__ == '__main__':
    ...
    pruned_layers = 2
    path_to_mask = f'fisher_pruning/outputs/head_mask_{pruned_layers - 1}.pt'

    if not os.path.exists('pruned_models'):
        os.makedirs('pruned_models')
    save_path = f'pruned_models/vitB16_pruned_sensitivity_{pruned_layers * 11}.pt'
    head_mask = torch.load(path_to_mask, map_location=torch.device('cpu'))

    pruned_model = CLIPModel_p.from_pretrained(model_name)
    tokenizer = CLIPProcessor.from_pretrained(model_name)

    # prune the model
    pruned_model = prune(pruned_model, head_mask)
